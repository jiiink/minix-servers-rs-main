...
static int sef_cb_init_fresh(int UNUSED(type), sef_init_info_t *UNUSED(info))
{
    ...
    nr_image_srvs = count_system_services(image, NR_BOOT_PROCS);
    ...
    reset_process_table();
    initialize_system_services(image);
    
    nr_uncaught_init_srvs = start_system_services();
...
static int count_system_services(struct boot_image *image, int max_procs)
{
    int count = 0;
    int i;
...
static void initialize_system_services(struct boot_image *image)
{
    int i;
    
    for (i = 0; boot_image_priv_table[i].endpoint != NULL_BOOT_NR; i++) {
        if (!iskerneln(_ENDPOINT_P(boot_image_priv_table[i].endpoint))) {
            setup_service(&boot_image_priv_table[i], image, i);
        }
    }
}

static void setup_service(struct boot_image_priv *boot_image_priv, 
                          struct boot_image *image, int index)
{
    struct rproc *rp;
    struct rprocpub *rpub;
    struct boot_image *ip;
    ...
    setup_privileges(rp, rpub, boot_image_priv, ip);
    setup_sys_properties(rpub, boot_image_sys);
    setup_dev_properties(rpub, boot_image_dev);
    setup_command_settings(rp, rpub, ip);
    setup_scheduling(rp, boot_image_priv);
    
    rpub->endpoint = ip->endpoint;
    initialize_defaults(rp);
...
static void setup_privileges(struct rproc *rp, struct rprocpub *rpub,
                             struct boot_image_priv *boot_image_priv,
                             struct boot_image *ip)
{
    int s;
...
static void setup_sys_properties(struct rprocpub *rpub, 
                                 struct boot_image_sys *boot_image_sys)
{
    rpub->sys_flags = boot_image_sys->flags;
}

static void setup_dev_properties(struct rprocpub *rpub,
                                 struct boot_image_dev *boot_image_dev)
{
    rpub->dev_nr = boot_image_dev->dev_nr;
}

static void setup_command_settings(struct rproc *rp, struct rprocpub *rpub,
                                   struct boot_image *ip)
{
...
static void setup_scheduling(struct rproc *rp, 
                             struct boot_image_priv *boot_image_priv)
{
...
static void initialize_defaults(struct rproc *rp)
{
    rp->r_old_rp = NULL;
    rp->r_new_rp = NULL;
    rp->r_prev_rp = NULL;
...
static void finalize_system_services(void)
{
    int i;
    struct boot_image_priv *boot_image_priv;
    struct rproc *rp;
...
static void perform_live_update(void)
{
    ...
    if (pid == 0) {
        handle_child_process(rp, replica_rp);
    } else {
        handle_parent_process(replica_rp, replica_endpoint);
    }
}

static void handle_child_process(struct rproc *rp, struct rproc *replica_rp)
{
    int s;
...
static void handle_parent_process(struct rproc *replica_rp, endpoint_t replica_endpoint)
{
    int s;
...
static int sef_cb_signal_manager(endpoint_t target, int signo)
{
    ...
    if (rs_verbose) {
        const char *signal_type = SIGS_IS_TERMINATION(signo) ? "termination" : "non-termination";
        printf("RS: %s got %s signal %d\n", srv_to_string(rp), signal_type, signo);
    }
